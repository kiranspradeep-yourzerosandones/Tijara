...
export default function Sidebar({ isCollapsed: externalCollapsed, setIsCollapsed: setExternalCollapsed }) {
  ...
  const [expandedSections, setExpandedSections] = useState([
    "main", "financial", "inventory", "reports", "system"
  ]);
    ...
    try {
      ...
      const responses = await Promise.allSettled([
        hasPermission("manageOrders")
          ? fetch(`${API_URL}/admin/orders/stats`, { headers })
          : Promise.resolve(null),
        hasPermission("manageProducts")
          ? fetch(`${API_URL}/products?stockStatus=low_stock`, { headers })
          : Promise.resolve(null),
      ]);
      ...
      if (responses[0].status === "fulfilled" && responses[0].value?.ok) {
        ...
        if (data.success) {
          setBadges((prev) => ({
            ...
            pendingCredits:
              data.data.stats?.pendingPaymentAmount > 0 ? "!" : 0,
          }));
        }
      }
      ...
      >
        ...
        {!isCollapsed && (
          <>
            ...
            {!isComingSoon && item.badge > 0 && (
              <span
                className={`flex items-center justify-center min-w-[20px] h-5 px-1.5 text-[10px] font-bold text-white rounded-full ${
        ...
        {isCollapsed && (
          <div className="absolute left-full ml-2 px-2.5 py-1.5 bg-gray-900 text-white text-xs font-medium rounded-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 whitespace-nowrap z-50 shadow-lg">
            {item.title}
            {isComingSoon && (
              <span className="ml-1 text-gray-400">(Coming Soon)</span>
            )}
          </div>
        )}
      </div>
    ...
    return (
      <span
        className={`px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider rounded-full ${
          styles[admin.role] || styles.admin
        }`}
      >
    ...
    return (
      <div className="flex flex-wrap gap-1 mt-2">
        {activePermissions.slice(0, 3).map((perm, idx) => (
          <span
            key={idx}
            className={`px-1.5 py-0.5 text-[9px] font-medium rounded ${perm.color}`}
          >
            {perm.label}
          </span>
        ))}
        ...
        {Object.keys(filteredNav).length <= 1 && !isCollapsed && (
          <div className="px-3 py-6 text-center">
            ...
            <p className="text-[10px] text-gray-400 mt-1">
              Contact super admin for more permissions
            </p>
          </div>
        )}
      </nav>
  ...
  return (
    <>
      ...
      {/* Mobile sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-72 bg-white shadow-2xl transform transition-transform duration-300 ease-out sm:hidden ${
      ...
      {/* Desktop sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 z-40 bg-white border-r border-gray-200 hidden sm:block transition-all duration-300 ease-out ${
