...
export default function EditProduct() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.title.trim()) { setError("Product title is required"); setSaving(false); return; }
    if (!formData.category.trim()) { setError("Category is required"); setSaving(false); return; }
          ...
          <div className="flex items-center justify-between py-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 hidden sm:block truncate max-w-xs">{formData.title || "Untitled"}</p>
              </div>
            </div>
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <button
                ...
                className={`px-5 sm:px-6 py-2.5 font-semibold transition-all flex items-center gap-2 rounded-lg ${
                  saving ? "bg-gray-200 text-gray-400 cursor-not-allowed" : "bg-amber-400 hover:bg-amber-500 text-gray-900 shadow-sm"
                }`}
              >
                {saving ? (
                  ...
                  <><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg><span>Update</span></>
                )}
              </button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-5 gap-6">
            ...
            <div className="xl:col-span-3 bg-white border border-gray-200">
              <div className="p-5 sm:p-6 space-y-5">
                ...
                {/* Short Description */}
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1.5">Short Description <span className="text-gray-400 font-normal">{formData.shortDescription.length}/200</span></label>
                ...
                {/* Storage - Rich Editor */}
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1.5">Storage / Specifications</label>
                  <button type="button" onClick={() => setStorageModal(true)}
                    ...
                    {formData.storage ? (
                      ...
                      <div className="px-4 py-4 flex items-center gap-3 text-gray-400 group-hover:text-amber-500 transition-colors">
                        ...
                        <span className="text-sm">Click to add storage instructions</span>
                      </div>
                    )}
                ...
                <label className="flex items-center justify-between py-3 px-4 bg-gray-50 border border-gray-200 cursor-pointer hover:bg-gray-100 transition-colors">
                  <div className="flex items-center gap-2">
                    <div className={`w-2 h-2 rounded-full ${formData.isActive ? "bg-emerald-500" : "bg-red-500"}`} />
                    <span className="text-sm font-medium text-gray-700">{formData.isActive ? "Active" : "Inactive"}</span>
                    <span className="text-xs text-gray-400">(visible on website)</span>
                  </div>
                  <div className="relative">
            ...
            <div className="xl:col-span-2 space-y-6">
              ...
              <div className="bg-white border border-gray-200">
                <div className="px-5 py-4 border-b border-gray-100">
                  <h3 className="text-sm font-semibold text-gray-900">Images <span className="text-gray-400 font-normal">{existingImages.length + newImagePreviews.length}/5</span></h3>
                </div>
                <div className="p-5">
                  {(existingImages.length > 0 || newImagePreviews.length > 0) && (
                    <div className="grid grid-cols-4 sm:grid-cols-5 xl:grid-cols-4 gap-2 mb-4">
                      {existingImages.map((img, index) => (
                        <div key={`ex-${index}`} className="relative aspect-square group cursor-pointer"
                          onClick={() => setPreviewImage(getImageUrl(img))}>
                          <ProductImage
                            src={img}
                            alt={`Product ${index + 1}`}
                            className="w-full h-full object-cover border border-gray-200 group-hover:border-amber-400 transition-colors"
                          />
                      ...
                      {newImagePreviews.map((preview, index) => (
                        <div key={`new-${index}`} className="relative aspect-square group cursor-pointer"
                          ...
                          <img src={preview} alt={`New ${index + 1}`}
                            className="w-full h-full object-cover border-2 border-emerald-400 group-hover:border-emerald-500 transition-colors" />
                          <span className="absolute top-1 left-1 px-1.5 py-0.5 bg-emerald-500 text-white text-[10px] font-bold">NEW</span>
                  ...
                  {(existingImages.length + newImagePreviews.length) < 5 && (
                    <label className={`block border-2 border-dashed border-gray-300 hover:border-amber-400 transition-colors cursor-pointer ${(existingImages.length + newImagePreviews.length) > 0 ? "p-4" : "p-8"}`}>
                      <input type="file" multiple accept="image/*" onChange={handleImageChange} className="hidden" />
                      <div className="flex flex-col items-center gap-2 text-gray-400">
                        <svg className={`${(existingImages.length + newImagePreviews.length) > 0 ? "w-6 h-6" : "w-10 h-10"}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                        </svg>
                        <span className="text-sm">{(existingImages.length + newImagePreviews.length) > 0 ? "Add more" : "Upload images"}</span>
                      </div>
                    </label>
                  )}
                </div>
              ...
              {/* Pricing */}
              <div className="bg-white border border-gray-200">
                <div className="px-5 py-4 border-b border-gray-100"><h3 className="text-sm font-semibold text-gray-900">Pricing</h3></div>
                    ...
                    <div className="flex gap-2">
                      <div className="flex-1 relative">
                        ...
                        <input type="number" name="price" value={formData.price} onChange={handleChange} placeholder="0.00" min="0" step="0.01"
                          className="w-full pl-7 pr-3 py-2.5 border border-gray-300 focus:ring-2 focus:ring-amber-400 focus:border-amber-400 text-gray-900" required />
                      </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input type="number" name="compareAtPrice" value={formData.compareAtPrice} onChange={handleChange} placeholder="Original price" min="0" step="0.01"
                        className="w-full pl-7 pr-3 py-2.5 border border-gray-300 focus:ring-2 focus:ring-amber-400 focus:border-amber-400 text-gray-900" />
                    </div>
                    {formData.price && formData.compareAtPrice && parseFloat(formData.compareAtPrice) > parseFloat(formData.price) && (
                      <p className="text-xs text-emerald-600 mt-1 font-medium">{Math.round(((parseFloat(formData.compareAtPrice) - parseFloat(formData.price)) / parseFloat(formData.compareAtPrice)) * 100)}% discount</p>
                    )}
                  </div>
                </div>
              </div>
              ...
              {/* Inventory */}
              <div className="bg-white border border-gray-200">
                <div className="px-5 py-4 border-b border-gray-100"><h3 className="text-sm font-semibold text-gray-900">Inventory</h3></div>
                <div className="p-5 space-y-4">
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input type="number" name="minOrderQuantity" value={formData.minOrderQuantity} onChange={handleChange} min="1"
                        className="w-full px-3 py-2.5 border border-gray-300 focus:ring-2 focus:ring-amber-400 focus:border-amber-400 text-gray-900" />
                    </div>
                    <div>
        ...
        <div className="xl:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-4 z-20">
          <div className="flex gap-3 ml-0 sm:ml-64">
            ...
            <button type="button" onClick={handleSubmit} disabled={saving}
              className={`flex-[2] py-3 font-semibold flex items-center justify-center gap-2 rounded-lg ${saving ? "bg-gray-200 text-gray-400" : "bg-amber-400 hover:bg-amber-500 text-gray-900"}`}>
