...
export default function AllProducts() {
  ...
  const fetchProducts = async () => {
    try {
      ...
      let productList = [];
      if (data.success && data.data?.products) productList = data.data.products;
  ...
  const handleDelete = async (id, title) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        await confirm({
          title: "Delete Failed",
          message: data.message || "Failed to delete product. Please try again.",
          confirmText: "OK",
          variant: "warning",
        });
      }
                    ...
                    <div className="flex gap-2 mt-3">
                      ...
                      <PermissionGate permission="manageProducts">
                        <button
                          ...
                          className="px-4 py-2.5 text-red-600 bg-red-50 rounded-xl text-sm
                                     font-medium hover:bg-red-100 transition-colors"
                        >
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
                ...
                >
                  <div className="relative aspect-square bg-gray-50">
                    ...
                    <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-all duration-300 flex items-center justify-center gap-3">
                      <Link
                        href={`/admin/products/view/${product.slug || product._id}`}
                        className="p-3.5 bg-white rounded-xl text-gray-700 hover:text-blue-600 transition-colors shadow-lg hover:scale-110"
                      >
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
                            d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                      ...
                      <PermissionGate permission="manageProducts">
                        <Link
                          href={`/admin/products/edit/${product._id}`}
                          className="p-3.5 bg-white rounded-xl text-gray-700 hover:text-amber-600 transition-colors shadow-lg hover:scale-110"
                        >
                          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
                              d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                      ...
                      <PermissionGate permission="manageProducts">
                        <button
                          onClick={() => handleDelete(product._id, product.title)}
                          className="p-3.5 bg-white rounded-xl text-gray-700 hover:text-red-600 transition-colors shadow-lg hover:scale-110"
                        >
                          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
                              d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5
