...
export default function ManageUsersPage() {
  ...
  useEffect(() => { fetchUsers(); }, [currentPage, statusFilter, creditFilter]);
  ...
  const handleToggleStatus = async (userId, currentStatus, userName) => {
    ...
    const ok = await confirm({
      title: `${currentStatus ? "Deactivate" : "Activate"} Customer`,
    ...
    try {
      ...
      await confirm({
        title: "Action Failed",
        message: err.message || "Could not update customer status.",
        confirmText: "OK",
        variant: "warning",
      });
    } finally {
  ...
  const handleDelete = async (userId, userName) => {
    const ok = await confirm({
      ...
      message: "This will permanently deactivate the customer account. All associated data will be retained for records.",
      details: userName,
      ...
      <div className="sticky top-0 z-30 -mx-6 px-6 bg-white/80 backdrop-blur-xl border-b border-gray-200/80">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 py-4 max-w-[1600px] mx-auto">
          <div>
            ...
            <p className="text-sm text-gray-400 mt-0.5">Manage customer accounts and credit</p>
          </div>
          <Link
            ...
            {(searchQuery || statusFilter !== "all" || creditFilter !== "all") && (
              <button
                ...
                onClick={() => {
                  ...
                  setCreditFilter("all"); setCurrentPage(1);
                }}
        ...
        <div className="bg-white rounded-2xl border border-gray-200/80 shadow-sm overflow-hidden">
          {error ? (
            <div className="text-center py-16">
              ...
              <button onClick={fetchUsers} className="text-blue-600 hover:text-blue-700 font-semibold">
                Try again
              </button>
            </div>
          ) : users.length === 0 ? (
                  ...
                  <div key={user._id} className="p-4">
                    <div className="flex gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {user.businessName && (
                          <p className="text-sm text-gray-600 mt-1 truncate">{user.businessName}</p>
                        )}
                    ...
                    <div className="mt-3 grid grid-cols-2 gap-3 text-sm">
                      ...
                      <div className="bg-orange-50 rounded-lg p-3">
                        ...
                        <p className="font-semibold text-orange-700">{formatCurrency(user.pendingAmount)}</p>
                      </div>
                      {user.creditBalance > 0 && (
                        <div className="col-span-2 bg-blue-50 rounded-lg p-3">
                          <p className="text-blue-600 text-xs">Credit Balance (Overpayment)</p>
                          <p className="font-semibold text-blue-700">{formatCurrency(user.creditBalance)}</p>
                        </div>
                      )}
                    </div>
              ...
              <div className="hidden sm:block overflow-x-auto">
                <table className="w-full">
                  <thead className="bg-gray-50/80 border-b border-gray-100">
                    <tr>
                      {["Customer", "Contact", "Business", "Credit", "Status", "Joined", ""].map((h, i) => (
                        <th
                          key={i}
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {users.map((user) => (
                      <tr key={user._id} className="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td className="px-6 py-4 text-gray-700 font-mono text-sm">{user.phone}</td>
                        <td className="px-6 py-4">
                          <p className="text-gray-900 truncate max-w-[150px]">{user.businessName || "—"}</p>
                          {user.businessType && (
                        ...
                        <td className="px-6 py-4">
                          <div className="text-sm">
                            <p className="text-gray-900 font-semibold">{formatCurrency(user.creditLimit)}</p>
                            <p className="text-orange-600">Pending: {formatCurrency(user.pendingAmount)}</p>
                            {user.creditBalance > 0 && (
                              <p className="text-blue-600 font-medium">Credit: {formatCurrency(user.creditBalance)}</p>
                            )}
                          </div>
                        </td>
                        <td className="px-6 py-4">
                          <div className="flex flex-col gap-1.5">
                            <span className={`inline-flex w-fit items-center px-2.5 py-0.5 rounded-lg text-xs font-medium border ${
                              user.isActive
                                ? "bg-emerald-50 text-emerald-700 border-emerald-200"
                                : "bg-red-50 text-red-700 border-red-200"
                            }`}>
              ...
              {pagination && pagination.pages > 1 && (
                <div className="px-6 py-4 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-4 bg-gray-50/50">
                  <p className="text-sm text-gray-600">
                    ...
                    <span className="text-gray-400 ml-2">({pagination.total} customers)</span>
                  </p>
