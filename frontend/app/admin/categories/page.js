...
export default function Categories() {
  ...
  const handleDelete = async (id, name) => {
    const ok = await confirm({
      ...
      message:
        "This will permanently delete the category. Products assigned to this category will become uncategorized.",
      details: `"${name}"`,
    ...
    try {
      ...
      if (res.ok) {
        ...
        await confirm({
          title: "Delete Failed",
          message: "Could not delete this category. It may be in use by products.",
          confirmText: "OK",
          variant: "warning",
        });
      }
              ...
              <div className="p-5 sm:p-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  {categories.map((category) => (
                    <div
                      key={category._id}
                      className={`group relative flex items-center justify-between p-4 rounded-xl border 
