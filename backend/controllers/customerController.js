  ...
  try {
    ...
    if (isCreditBlocked !== undefined) {
      ...
      if (isCreditBlocked) {
        ...
        customer.creditBlockedBy = req.user._id;
      } else {
...
exports.getCustomerStats = async (req, res) => {
  try {
    ...
    const [
      ...
      User.countDocuments({ role: "customer", lastLoginAt: { $gte: thirtyDaysAgo } }),
      User.countDocuments({ role: "customer", pendingAmount: { $gt: 0 } }),
      User.countDocuments({ role: "customer", isCreditBlocked: true }),
      User.aggregate([
        { $match: { role: "customer", businessType: { $exists: true, $ne: null, $ne: "" } } },
...
/**
 * @desc    Delete customer (soft delete by default, hard delete optional)
...
exports.deleteCustomer = async (req, res) => {
  try {
    ...
    // Prevent hard delete if customer has pending orders
    if (hardDelete && pendingOrders > 0) {
      return res.status(400).json({
        success: false,
        message: `Cannot permanently delete customer with ${pendingOrders} pending orders. Cancel orders first or use soft delete.`
      });
    }
    ...
    if (hardDelete) {
      ...
      if (orderCount > 0) {
        return res.status(400).json({
          ...
          suggestion: "Set hardDelete=false to deactivate instead"
        });
      }
      ...
      console.log(`🗑️ HARD DELETE: Customer ${customer.phone} permanently deleted by admin`);
    ...
    // ✅ SOFT DELETE (recommended - just deactivate)
